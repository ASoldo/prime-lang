...
- UART logging: `out(...)` prints over UART via ROM `ets_printf`. Strings, format strings, ints (constants), and bools are rendered; each `out` appends its own newline so successive calls separate cleanly. The blink demo emits your `out(...)` lines as written—no extra boot banner is injected by the runtime.
...
2. Run the demo: `prime-lang build workspace/demos/esp32_blink/esp32_blink.prime --name esp32_blink`
   - The runtime prints your `out(...)` lines on UART (115200) with newlines per call, and the on-board blue LED (GPIO2, active-low) blinks.
