...
- Host vs no_std feature set: `fs_exists`/`fs_read`/`fs_write` are host-only. Embedded/no_std keeps `out`, channels, async timers, GPIO, mutable scalars, and now `spawn`/`join` (mapped to the task runtime, not OS threads); set `no_std = true` in the manifest (see `workspace/demos/esp32_blink/prime.toml` or `workspace/demos/bare_metal_embedded/prime.toml`). The LSP/Tree-sitter tooling reads the same manifest, so host-only completions stay hidden when editing embedded targets while GPIO/delay/reset helpers are suggested.
